...
export default function Reports({ members, months, currentMonth, onUpdateMembers, onUpdateMonths }) {
  ...
  function handleMeetingReport() {
    ...
    const html = '<!DOCTYPE html><html><head><meta charset="UTF-8">'
      ...
      + '<td style="padding:10px;text-align:right;font-weight:700;">' + formatRs(members.reduce((s,mb)=>{const e=monthRecord.entries[mb.id];return e?s+(e.saving||0):s;},0)) + '</td>'
      + '<td style="padding:10px;text-align:right;font-weight:700;">' + formatRs(members.reduce((s,mb)=>{const e=monthRecord.entries[mb.id];return e?s+(e.principal||0):s;},0)) + '</td>'
      + '<td style="padding:10px;text-align:right;color:#C62828;font-weight:700;">' + formatRs(members.reduce((s,mb)=>{const e=monthRecord.entries[mb.id];return e?s+(e.interest||0):s;},0)) + '</td>'
      + '<td></td>'
