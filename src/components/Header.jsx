export default function Header({ activeTab, setActiveTab, onLock, group }) {
      ...
      <div style={{ background: "linear-gradient(135deg, #E8650A 0%, #C4540A 50%, #A03A06 100%)", padding: "16px 20px 20px", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 12px rgba(0,0,0,0.18)" }}>
        <div style={{ position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div className="marathi" style={{ color: "#fff", fontSize: 15, fontWeight: 700, lineHeight: 1.4 }}>{group?.name || "BachatMitra"}</div>
              <div className="marathi" style={{ color: "rgba(255,255,255,0.8)", fontSize: 12 }}>{group?.address || ""}</div>
            </div>
