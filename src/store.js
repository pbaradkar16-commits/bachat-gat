...
const KEYS = { members: "sssb_members", months: "sssb_months", currentMonth: "sssb_current_month", bankBalance: "sssb_bank_balance", groupInfo: "sssb_group_info" };
...
export function loadMembers() {
  ...
  const members = DEFAULT_MEMBERS.map((name,i) => ({ id:genId()+i, name, phone:"", loanAmount:0, balance:0, loanHistory:[], createdAt:Date.now() }));
...
export function loadBankBalances() { return load(KEYS.bankBalance) || {}; }
export function saveBankBalances(b) { save(KEYS.bankBalance, b); }
export function createMonthRecord(members, key) {
  ...
  return { key, entries, createdAt:Date.now(), locked:false, bankBalance:0, notes:"" };
}
...
export function getGroupBalanceSheet(members, months, bankBalance) {
  const totalLoansGiven = members.reduce((s,m)=>s+m.loanAmount,0);
  ...
  return { totalLoansGiven, totalOutstanding, totalSavingsCollected, totalInterestEarned, totalCollection, activeLoans, completedLoans, bankBalance, netWorth: bankBalance+totalOutstanding };
}
